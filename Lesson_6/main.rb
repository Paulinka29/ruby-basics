...
class Main1
  ...
  def create_route
    ...
    if first_station_object && last_station_object
  ...
  def create_train
    ...
    train_object = trains.find { |train| train.number == number && train.type == type_class }
    train = case type
            when 1
    ...
    if train_object
      puts 'Поезд с таким именем и типом уже существует'
    else
      trains << train
  ...
  def manage_trains_routes
    ...
    train  = trains.find { |train| train.number == number && train.type == type_class }
